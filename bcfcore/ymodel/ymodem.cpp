...
YModem::Code YModem::receivePacket()
{
    if (code == CodeNone) {
        ...
        if (code == CodeSoh) {
            ...
            uint32_t len = read(&(rxBuffer[rxLength]),
                                YMODEM_PACKET_1K_SIZE + YMODEM_PACKET_OVERHEAD - rxLength);
...
void YModem::receiveStageTransmitting()
{
    switch (receivePacket()) {
        case CodeSoh: {
                ...
                } else if ((rxBuffer[1] == (uint8_t)(dataCount + 1))
                           && (rxBuffer[2] == (uint8_t)(0xFE - dataCount)) &&
                           (crc == crc16(&(rxBuffer[YMODEM_PACKET_HEADER]), YMODEM_PACKET_SIZE))) {
                    uint32_t dataLength = YMODEM_PACKET_SIZE;
    ...
    switch (receivePacket()) {
        ...
        default: {
                ...
                if ((timeCount / (timeDivide + 1)) > timeMax) {
                    ...
                    callback(StatusEstablishing, NULL, NULL);
            }
    }
}
